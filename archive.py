...
def paper_k_nmt_appx_match(event_para, sig_para, appr_para):
    ...
    for i in range(k):
        ...
        while Q:
            H = Q[0]
            Q = list_move_pop(Q)
            D_for_match = ['S' for i in range(m)]  # modified
            D_for_match[H[0]:H[-1]+1] = event[H[0]:H[-1]+1]  # modified
            D = event[H[0]:H[-1]+1]  # modified
            D_start_index = H[0]
            D_end_index = H[-1]
            ...
            for L_item in L[i]:
                ...
                B = D[0:L_item[-1]]
                D_for_match = ['S' for i in range(m)]  # modified
                D_for_match[L_item[0]+1::] = D[L_item[0]+1::]  # modified
                D = D[L_item[0]+1::]  # modified
                print("D second:", D)
                if B:
                    start = B_start_index
                    end = B_end_index
